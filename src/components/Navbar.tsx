...
import { Menu, X, Snowflake, ChevronRight } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-12">
          ...
          <div className="hidden md:flex items-center space-x-1">
            <div className="flex space-x-1 bg-white/5 border border-white/5 rounded-full px-2 py-1 backdrop-blur-md mr-4">
              {navLinks.map((link) => (
                <Link 
                  key={link.name} 
                  href={link.path}
                  className="relative px-4 py-2 text-sm font-medium text-gray-300 hover:text-cyan-300 transition-colors rounded-full overflow-hidden group"
                >
                  <span className="relative z-10 tracking-wide uppercase text-xs">{link.name}</span>
                  <div className="absolute inset-0 bg-cyan-500/0 group-hover:bg-cyan-500/10 transition-colors rounded-full"></div>
                </Link>
              ))}
            </div>
        ...
        {isOpen && (
          ...
          >
            <div className="px-4 py-6 space-y-2 flex flex-col items-center">
              ...
              <Link 
                href="/contact"
                className="block w-full text-center mt-6 px-5 py-4 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold uppercase tracking-widest text-sm hover:shadow-[0_0_20px_rgba(6,182,212,0.4)] transition-all"
                onClick={() => setIsOpen(false)}
              >
                Initiate Systems
              </Link>
